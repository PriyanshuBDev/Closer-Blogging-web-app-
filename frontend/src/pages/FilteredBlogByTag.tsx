...
import { useBlogByTagFilter } from "../hooks/filter";
import { useParams } from "react-router-dom";

export function FilteredBlogByTag() {
  const [page, setPage] = useState(1);
  const { filter } = useParams();
  const currentFilter = filter ?? "";
  const { loading, blogs, total, totalPages } = useBlogByTagFilter({
        ...
        {loading ? (
          ...
          <div className="mt-2">
            {blogs && blogs.length > 0 ? (
              ...
              <div className="mt-4 text-gray-500">
                No blogs found for "{currentFilter}"
              </div>
            )}
            <Pagination
              setPage={setPage}
